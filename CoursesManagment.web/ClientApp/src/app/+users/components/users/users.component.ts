...
export class UsersComponent extends BaseComponent implements OnInit {
  ...
  disabledExport() {
    // return this.usersTableComponent?.tableService?.dataList?.length == 0
  }

  get showCreateDropdowne(): boolean {
    return this.form.controls['role'].value == this.userRolesEnum.Admin
  }
  ...
  createUser(role: RolesEnum) {
    ...
    modalRef
      .result
      .then((actionCompleted: boolean) => !actionCompleted || this.activeModal.close(true) || this.usersTableComponent.loadUsers()).catch(() => {
      });
  }

  isCurrentUserSuperAdmin() {
    return this.currentUser.roles.includes(RolesEnum[RolesEnum.Admin]);
  }
