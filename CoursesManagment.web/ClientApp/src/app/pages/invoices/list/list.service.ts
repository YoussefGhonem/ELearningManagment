...
export class ListService {
  ...
  constructor(private pipe: DecimalPipe, public ProductService: ProductService, private datePipe: DatePipe) {
    ...
    // Api Data
    this.ProductService.getInvoiceData().subscribe(
      data => {        
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 4. Date Filter       
    if(date){
      countries = countries.filter(country => this.datePipe.transform(country.date,"yyyy-MM-dd") == this.datePipe.transform(date,"yyyy-MM-dd"));
    }
    else{
      countries = countries;
    }
