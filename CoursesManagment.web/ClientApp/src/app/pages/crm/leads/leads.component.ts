...
export class LeadsComponent {
  ...
  constructor(private modalService: NgbModal,public service: LeadsService, private formBuilder: UntypedFormBuilder, private ProductService : ProductService, private offcanvasService: NgbOffcanvas, private datePipe: DatePipe) {
  ...
  */
   saveUser() {
    if (this.leadsForm.valid) {
      if (this.leadsForm.get('ids')?.value) {
        this.ProductService.patchLeadData(this.leadsForm.value).subscribe(
          (data: any) => {
        ...
        this.ProductService.postContactData(this.leadsForm.value).subscribe(
          (data: any) => {
   ...
   */
   editDataGet(id: any, content:any) {    
    ...
    this.ProductService.getSingLeadData(id).subscribe({
      next: data => {    
   ...
   */
   ...
   deleteData(id:any) {    
    if(id){
      this.ProductService.deletelead(id).subscribe({
        next: data => { },
        error: err => {
          this.content = JSON.parse(err.error).message;
        }
