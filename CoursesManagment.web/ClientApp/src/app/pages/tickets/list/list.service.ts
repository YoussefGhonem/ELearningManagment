...
export class ListService {
  ...
  constructor(private pipe: DecimalPipe, public ProductService: ProductService, private datePipe: DatePipe) {
    ...
    // Api Data
    this.ProductService.getTicketData().subscribe(
      data => {        
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 4. Date Filter       
    if(date){
      lists = lists.filter(country => this.datePipe.transform(country.create,"yyyy-MM-dd") == this.datePipe.transform(date,"yyyy-MM-dd"));
    }
    else{
      lists = lists;
    }
