...
export class ListViewComponent {
  ...
  constructor(private modalService: NgbModal,public service: ListViewService, private formBuilder: UntypedFormBuilder, private ProductService : ProductService, private datePipe: DatePipe) {
  ...
  */
  saveUser() {
    if (this.tasksForm.valid) {
      if (this.tasksForm.get('ids')?.value) {
        this.ProductService.patchOrderData(this.tasksForm.value).subscribe(
          (data: any) => {
        ...
        this.ProductService.postTaskData(this.tasksForm.value).subscribe(
          (data: any) => {              
   ...
   */
   editDataGet(id: any, content:any) {
    ...
    this.ProductService.getSingleTaskData(id).subscribe({
      next: data => {    
  ...
  deleteData(id:any) {    
    if(id){
      this.ProductService.deleteTask(id).subscribe({
        next: data => { },
        error: err => {
          this.content = JSON.parse(err.error).message;
        }
