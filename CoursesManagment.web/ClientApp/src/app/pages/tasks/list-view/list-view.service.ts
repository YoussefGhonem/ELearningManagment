...
function matches(country: Country, term: string, pipe: PipeTransform) {
  return country.creater.toLowerCase().includes(term.toLowerCase())
...
export class ListViewService {
  ...
  constructor(private pipe: DecimalPipe, public ProductService: ProductService, private datePipe: DatePipe) {
    this._search$.pipe(
    ...
    // Api Data
    this.ProductService.getTaskData().subscribe(
      data => {        
  ...
  private _search(): Observable<SearchResult> {
    ...
    //4. Date Filter           
    if(date){
      customers = customers.filter(country => this.datePipe.transform(country.dueDate,"yyyy-MM-dd") == this.datePipe.transform(date,"yyyy-MM-dd"));
    }
    else{
      customers = customers;
    }
